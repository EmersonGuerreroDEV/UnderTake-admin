...
interface TableProps {
  allUser:UserRol[]
  isLoading:boolean
  selectUser: (e:UserRol)=>void
}

const TableUsers = ({allUser, isLoading,selectUser}:TableProps) => {
            ...
            {allUser.map((user, index) => (
              <tr key={index}>
                ...
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3  ${index === packageData.length - 1 ? "border-b-0" : "border-b"}`}
                >
                  <button onClick={()=>selectUser(user)} className="text-dark w-full mx-auto text-center dark:text-white">
                  <Eye />
                  </button>
                </td>
              </tr>
